        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-9 ">
          {SkillsInfo.map((curSkill) => {
            return (
              ...
              >
                {/* Title */}
                <h1 className="text-center w-full mb-3  text-2xl text-gray-400 font-bold ">
                  {curSkill.title}
                </h1>
                <Tilt
                  tiltMaxAngleX={20}
                    ...
                    {curSkill.skills.map((curElem) => {
                      return (
                        <motion.li
                          ...
                          whileInView="show"
                          key={curElem.name}
                          className="border border-gray-700 rounded-full px-4 py-2 flex w-full gap-2 items-center justify-center  "
                        >
                          {curElem.logo && (
                            <img
                              src={curElem.logo}
                              alt={`${curElem.logo} Logo`}
                              className="w-8 h-8"
                            />
                          )}
